            ...
            .with(|boa_context_ref_cell| {
                ...
                let function_js_object = function_js_value
                    ...
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "searchCars"),
                    ))?;
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "returnCar"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addCar"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[_cdk_user_defined_car.try_into_vm_value(&mut boa_context)?],
                        &mut boa_context,
                    )?;
                let final_return_value = async_await_result_handler(
                    &mut boa_context,
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateCar"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deleteCar"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
